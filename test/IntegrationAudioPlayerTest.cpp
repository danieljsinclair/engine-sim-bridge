...
// Platform-specific factory coverage: AudioHardwareProviderFactory::createProvider
// selects the concrete IAudioHardwareProvider for the build platform. On macOS this
// must return a CoreAudio-backed provider; the factory source is otherwise dead-stripped
// (no other test routes through it). Guard mirrors the CoreAudio build guard.
#if defined(__APPLE__) && defined(__MACH__)
#include <TargetConditionals.h>
#endif
#if defined(__APPLE__) && defined(__MACH__) && TARGET_OS_MAC && !TARGET_OS_IPHONE
TEST_F(AudioStrategyIntegrationTest, HardwareFactory_CreatesCoreAudioProvider) {
    auto provider = AudioHardwareProviderFactory::createProvider(logger_.get());
    ASSERT_NE(provider, nullptr);

    // Provider must accept a callback + initialize like the direct CoreAudio path.
    auto mockCallback = [](AudioBufferView& buffer) -> int {
        (void)buffer;
        return 0;
    };
    EXPECT_TRUE(provider->registerAudioCallback(mockCallback));
