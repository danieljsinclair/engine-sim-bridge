...
void ExhaustSystemDeserializer::deserialize(const JsonValue& json, ExhaustSystem* es,
    ...
    if (!json.has("audioVolume")) {
        throw PresetDeserializationException("Missing required field 'audioVolume' in " + ctx);
    }
    ...
    if (!json.has("collectorCrossSectionArea")) {
        throw PresetDeserializationException("Missing required field 'collectorCrossSectionArea' in " + ctx);
    }
    params.collectorCrossSectionArea = json["collectorCrossSectionArea"].asNumber();

    if (!json.has("outletFlowRate")) {
        throw PresetDeserializationException("Missing required field 'outletFlowRate' in " + ctx);
    }
    params.outletFlowRate = json["outletFlowRate"].asNumber();

    if (!json.has("primaryTubeLength")) {
        throw PresetDeserializationException("Missing required field 'primaryTubeLength' in " + ctx);
    }
    params.primaryTubeLength = json["primaryTubeLength"].asNumber();

    if (!json.has("primaryFlowRate")) {
        throw PresetDeserializationException("Missing required field 'primaryFlowRate' in " + ctx);
    }
    ...
    if (json.has("impulseResponseFilename") && json["impulseResponseFilename"].isString()) {
        ...
        if (!json.has("impulseResponseVolume")) {
            throw PresetDeserializationException("Missing required field 'impulseResponseVolume' when 'impulseResponseFilename' present in " + ctx);
        }
